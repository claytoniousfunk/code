...
// V8 : new pthat exclusion logic (from M Ngyuen ).  Added pthat regions histograms from V4.
...
void makeDataV8(bool isReco = 0, double muPtCut = 5.0){
	...
	for (evi = 0; evi < n_evts; evi++){
     ...
     ///////////////////////////////////////////////////////////////////   Calculate weight (pthat)   //////////////////////////////////////////////////////////////////////////
		...
		for(int jetj=0; jetj < (int)pf_jteta->size(); jetj++){
			...
			h_vz->Fill(vz,w);
			h_vz_noWeight->Fill(vz,weight*w2);

			h_hiBin->Fill(hiBin,w);
			h_hiBin_noWeight->Fill(hiBin,weight*w1);
