...
V4 :  implementing variable binning
*/
...
void jetPtFractionV4(){
	...
	double pThi = 600.;
	int Nbins = 0;
	vector <float> *edges=0;
	double delta = 5; // initial width of bin
	double jtpt_bindouble1 = 350;
	double jtpt_bindouble2 = 500;
	double jtpt_check=pTlow;
	edges[0]==pTlow;
	for(int zz=1; jtpt_check<pThi; zz++){
		if(jtpt_check<jtpt_bindouble1){
			edges[zz]=pTlow + zz*delta;
			jtpt_check=jtpt_check+delta;
			Nbins=Nbins+1;
		}
		if(jtpt_check>jtpt_bindouble1 && jtpt_check<jtpt_bindouble2){
			edges[zz]=pTlow+2*zz*delta;
			jtpt_check=jtpt_check+2*delta;
			Nbins=Nbins+1;
		}
		if(jtpt_check>jtpt_bindouble2){
			edges[zz]=pTlow+4*zz*delta;
			jtpt_check=jetpt_check+4*delta;
			Nbins=Nbins+1;
		}
	...
	*/
	//Nbins=100;
	const Int_t Nbins= 66;
	Double_t edges[Nbins+1]={120, 125, 130, 135, 140, 145, 150, 155, 160, 165, 170, 175, 180, 185, 190, 195, 200, 205, 210, 215, 220, 225, 230, 235, 240, 245, 250, 255, 260, 265, 270, 275, 280, 285, 290,
		295, 300, 305, 310, 315, 320, 325, 330, 335, 340, 345, 350, 360, 370, 380, 390, 400, 410, 420, 430, 440, 450, 460, 470, 480, 490, 500, 520, 540, 560, 580, 600};
	...
	TH1D *h_jetpt = new TH1D("h_jetpt","Jet pT",Nbins,edges);
		// parse jetpt by species of particles
		TH1D *h_jetpt_d = new TH1D("h_jetpt_d","",Nbins,edges); // down quark
      		TH1D *h_jetpt_u = new TH1D("h_jetpt_u","",Nbins,edges); // up quark
      		TH1D *h_jetpt_s = new TH1D("h_jetpt_s","",Nbins,edges); // strange quark
      		TH1D *h_jetpt_c = new TH1D("h_jetpt_c","",Nbins,edges); // charm quark
      		TH1D *h_jetpt_b = new TH1D("h_jetpt_b","",Nbins,edges); // bottom quark
      		TH1D *h_jetpt_dbar = new TH1D("h_jetpt_dbar","",Nbins,edges); // down antiquark
      		TH1D *h_jetpt_ubar = new TH1D("h_jetpt_ubar","",Nbins,edges); // up antiquark
      		TH1D *h_jetpt_sbar = new TH1D("h_jetpt_sbar","",Nbins,edges); // strange antiquark
      		TH1D *h_jetpt_cbar = new TH1D("h_jetpt_cbar","",Nbins,edges); // charm antiquark
      		TH1D *h_jetpt_bbar = new TH1D("h_jetpt_bbar","",Nbins,edges); // bottom antiquark
      		TH1D *h_jetpt_g = new TH1D("h_jetpt_g","",Nbins,edges); // gluon
      		TH1D *h_jetpt_hq = new TH1D("h_jetpt_hq","",Nbins,edges); // all heavy quarks (s,c,b,s-bar,c-bar,b-bar)
		TH1D *h_jetpt_other = new TH1D("h_jetpt_other","",Nbins,edges); // others
	...
	// Write data	
auto wf = TFile::Open("jetPtFractionV4.root", "recreate");
c->SaveAs("jetPtFractionV4.pdf");
h_jetpt->Write();
h_jetpt_d->Write();
h_jetpt_u->Write();
h_jetpt_s->Write();
