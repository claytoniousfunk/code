...
void jetParticleFraction(){

	/// LOAD DATA ///
        auto f =TFile::Open("/home/clayton/Analysis/data/HiForestAOD_PbPbMC2018skim_100.root");
	...
	for (evi = 0; evi < n_evts; evi++){
		...
		for(int jeti = 0; jeti < em->njet; jeti++){
			...
			double x = em->jetpt[jeti];
			double u = em->jeteta[jeti];
			int y = em-> flavor_forb[jeti];
			if(y==0){h_jetpt_other -> Fill(x); continue;}
			if(fabs(u)>=etamaxcut || x <= pTmincut){continue;}
			h_jetpt -> Fill(x);
	...
	h7->SetFillColor(kBlue-10);
       h1 -> SetStats(0);
       h1 -> GetXaxis() -> SetTitle("Jet pT (GeV/c)");
	...
	*/
	h_jetpt->Draw("hist");
	h_jetpt -> GetXaxis() -> SetTitle("pT (GeV/c)");
