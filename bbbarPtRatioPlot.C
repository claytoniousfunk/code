...
void bbbarPtRatioPlot(){
    ...
    TH1D *h1 = (TH1D*)h_jetpt_b->Clone("h1"); // u quarks
    TH1D *h2 = (TH1D*)h_jetpt_bbar->Clone("h2"); // d quarks
    ...
    legend->AddEntry(h1,"b","fp");
    legend->AddEntry(h2,"#bar{b}","fp");
    legend->Draw();
   ...
   h3->GetYaxis()->SetTitle("ratio b/#bar{b}");
