...
void ubardbarPtRatioPlot(){
	/// LOAD DATA ///
    TFile* f =TFile::Open("/home/clayton/Analysis/code/jetPtFractionV6.root");
    TH1D* h_jetpt_ubar;
    TH1D* h_jetpt_dbar;
    f->GetObject("h_jetpt_ubar",h_jetpt_ubar);
    f->GetObject("h_jetpt_dbar",h_jetpt_dbar);
    
    TH1D *h1 = (TH1D*)h_jetpt_ubar->Clone("h1"); // u quarks
    TH1D *h2 = (TH1D*)h_jetpt_dbar->Clone("h2"); // d quarks
    ...
    legend->AddEntry(h1,"#bar{u}","fp");
    legend->AddEntry(h2,"#bar{d}","fp");
    legend->Draw();
    ...
    // h1 settings
    h1->SetFillColorAlpha(kMagenta-5,0.5);
    h1->SetMarkerStyle(8);
    h1->SetMarkerColor(kMagenta-1);
   ...
   // h2 settings
   h2->SetFillColorAlpha(kSpring-4,0.5);
   h2->SetLineWidth(1);
   h2->SetMarkerStyle(22);
   h2->SetMarkerColor(kSpring-6);
   ...
   h3->GetYaxis()->SetTitle("ratio #bar{u}/#bar{d} ");
   ...
   c->SaveAs("ubardbarPtRatioPlot.pdf");
}