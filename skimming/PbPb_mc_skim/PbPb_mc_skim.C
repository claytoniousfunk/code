	...
	//for(int evi = 1; evi < 5; evi++){	
		...
		for(int jetj = 0; jetj < em->njet; jetj++){
			...
			if(em->hadronFlavor[jetj]==4){
				...
				if(em->partonFlavor[jetj]==1 || em->partonFlavor[jetj]==-1 || em->partonFlavor[jetj]==2 || em->partonFlavor[jetj]==-2 || em->partonFlavor[jetj]==3 || em->partonFlavor[jetj]==-3){
...
auto wf = TFile::Open("/uscms/home/cmbennet/work/PbPb_mc_skim_noWeights_pthat30_muptcut10_2Sep20.root","recreate");
