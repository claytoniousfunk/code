...
void ccbarPtRatioPlot(){
    ...
    TH1D *h1 = (TH1D*)h_jetpt_c->Clone("h1"); // c quarks
    TH1D *h2 = (TH1D*)h_jetpt_cbar->Clone("h2"); // c antiquarks
    ...
    legend->AddEntry(h1,"c","fp");
    legend->AddEntry(h2,"#bar{c}","fp");
    legend->Draw();
   ...
   h3->GetYaxis()->SetTitle("ratio c/#bar{c}");
