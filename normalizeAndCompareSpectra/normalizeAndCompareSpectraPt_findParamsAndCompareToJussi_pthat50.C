...
void normalizeAndCompareSpectraPt_findParamsAndCompareToJussi_pthat50(){
...
///////////////////////////////////////////////////////////////////////////  ratio plot  ////////////////////////////////////////////////////////////////////////////
   ...
   TH1D *jussi_pt_ynorm = (TH1D*) jussi_pt->Clone("jussi_pt_ynorm");
   jussi_pt_ynorm->Scale(1.0/jussi_pt_ynorm->Integral());
   TH1D *jussi_pt_ynorm_rebin = (TH1D*) jussi_pt_ynorm->Rebin(15,"jussi_pt_ynorm_rebin",pt_axis);
   TH1D *jussi_pt_ynorm_rebin_xnorm = (TH1D*) jussi_pt_ynorm_rebin->Clone("jussi_pt_ynorm_rebin_xnorm");
   int N_jussi = jussi_pt_ynorm_rebin_xnorm->GetSize();
   for(int i=0;i<N_jussi;i++){
       double w = jussi_pt_ynorm_rebin_xnorm->GetBinWidth(i);
       double y = jussi_pt_ynorm_rebin_xnorm->GetBinContent(i);
       double e = jussi_pt_ynorm_rebin_xnorm->GetBinError(i);
       if(w!=0){
           jussi_pt_ynorm_rebin_xnorm->SetBinContent(i,y/w);
           jussi_pt_ynorm_rebin_xnorm->SetBinError(i,e/w);
       }
   }
  
   TCanvas *c4 = new TCanvas("c4","Reweighted pt vs Jussi pt",500,500);
    c4->cd();
    TPad *pad5 = new TPad("pad5","pad5",0.0,0.3,1.0,1.0);
    ...
    jussi_pt_ynorm_rebin_xnorm->SetMarkerStyle(24);
    jussi_pt_ynorm_rebin_xnorm->SetMarkerColor(kBlue);
    jussi_pt_ynorm_rebin_xnorm->SetFillColorAlpha(kBlue,0.7);
    jussi_pt_ynorm_rebin_xnorm->GetYaxis()->SetTitle("1/N^{jet} dN^{jet}/dp_{T}^{jet}");
    jussi_pt_ynorm_rebin_xnorm->GetYaxis()->SetTitleSize(0.06);
    jussi_pt_ynorm_rebin_xnorm->SetTitle("");
    jussi_pt_ynorm_rebin_xnorm->SetStats(0);

    jussi_pt_ynorm_rebin_xnorm->Draw("e2");
    pythia_pt_xnorm_fitscaled_xnorm->Draw("e2 same");

   auto legend3 = new TLegend(0.5,0.65,0.89,0.82);
   legend3->AddEntry(jussi_pt_ynorm_rebin_xnorm,"Jussi","p");
   legend3->AddEntry(pythia_pt_xnorm_fitscaled_xnorm,"Reweighted pythia","p");
   legend3->SetBorderSize(0);
   legend3->Draw();

   pad6->cd();
   TH1D *r3 = (TH1D*) pythia_pt_xnorm_fitscaled_xnorm->Clone("r3");
   r3->Divide(jussi_pt_ynorm_rebin_xnorm);
   ...
   r3->SetTitle("");
   r3->SetStats(0);
   r3->GetXaxis()->SetTitle("p_{T}^{jet} [GeV/c]");
   r3->GetYaxis()->SetTitle("Clayton / Jussi");
   r3->GetXaxis()->SetTitleSize(0.10);
   r3->GetYaxis()->SetTitleSize(0.10);
   r3->GetXaxis()->SetLabelSize(0.08);
   r3->GetYaxis()->SetLabelSize(0.08);
   r3->GetYaxis()->SetTitleOffset(0.6);

   c4->SaveAs("/home/clayton/Analysis/code/normalizeAndCompareSpectra/figures/pythiaDataPtRescaleCompare_findParamsAndCompareToJussi_pthat50.pdf");
